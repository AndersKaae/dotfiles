...
- **Commit hygiene:** focused commits with `Task`/area prefix; commit rebuilt artifacts and any new design assets; revert server-regenerated drift (e.g. `appsettings-schema.*`). The Vue bundle under `wwwroot/scripts/vue` is git-ignored — don't commit it. **No `Co-Authored-By: Claude ...` trailer and no `🤖 Generated with Claude Code` line** — this repo does not want Claude attribution on commits or PR descriptions, overriding the harness default.
